...
**Fuentes:** `PRD-Plataforma-Gamificada-TP.md` (definición funcional), `temas_curso.md` (asignación de temas).
...
| | `ms-cursos` | T02 | Curso, comisión, padrón, inscripción, código de invitación, estado del curso |
...
| | `ms-progreso` | T10 | Grafo/roadmap, desbloqueo, XP, niveles, logros, ranking |
| **Interacción y gobierno** | `ms-social` | T11 | Chat, equipos, notificaciones, reportes, moderación |
| | `ms-backoffice` | T12 | Configuración global (PAR-01..18), asignación modelo↔función |
...
Los 18 parámetros de la Sección 4.1 del PRD los consume medio sistema. Opciones: Spring Cloud Config, o que `ms-backoffice` los exponga y cada servicio cachee con TTL + evento `ParametroCambiado` para invalidar.
...
Con 12 grupos y 132 personas, el cuello de botella no va a ser técnico sino de integración. Cinco decisiones que idealmente son conjuntas y no de cada grupo:

1. **Catálogo de eventos compartido**, con envelope estándar: `eventId`, `tipo`, `version`, `ocurridoEn`, `correlationId`, `causationId`, `actor`, `payload`. Publicado como AsyncAPI en un repo común.
...
5. **Quién es dueño de qué dato**, resuelto explícitamente y por escrito. Las tres fronteras calientes:
   - **vidas** → Banco (T08) vs. Desafíos (T03)
   - **XP** → Progreso (T10) vs. Eval-IA (T07)
   - **inventario** → Mercado (T09) vs. Progreso (T10)
