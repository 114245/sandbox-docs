...
| Documento oficial de arquitectura | Pendiente; lo de acá se revalida cuando llegue |
...
**Lo que esto no cubre:** el kernel es compartido. Un exploit de kernel escapa del contenedor. Para el alcance del TP es un riesgo aceptable — pero hay que **documentarlo explícitamente** como residual asumido, no ignorarlo.
...
> Esta secuencia está **verificada contra Docker real** con diez entregas de prueba, seis de ellas hostiles. Los dos cambios respecto de la versión anterior de este documento —la entrada por `stdin` y el reloj partido en dos— salieron de ahí, no de la teoría. Ver §1.4.
...
```
 ...
 4. docker create -i --network none --read-only --user 10001:10001
                  ...
                  -e TIMEOUT_COMPILE_MS=20000     ← reloj de la PLATAFORMA
                  -e TIMEOUT_TESTS_MS=10000       ← reloj del ALUMNO
                  java-runner:21
 ...
 6. El script corre las fases y le pone reloj a cada una:
      ...
      (un `timeout` por fase; el alumno solo controla la tercera)
...
- **El reloj está partido en dos, y solo uno es del alumno.** Compilar es costo de plataforma; el porqué está medido en §1.4.
...
java -jar /opt/junit/junit.jar execute \
     --class-path /tmp/classes --scan-class-path=/tmp/classes \
...
**`--scan-class-path` apunta al directorio de clases, no al classpath completo.** Dejarle escanear también el jar de JUnit lleva el descubrimiento de ~1.2 s a ~2.9 s (§1.4a).
...
#### d) El código del alumno y los tests corren en la misma JVM

El alumno puede, en teoría, tocar el runner por reflection. Se mitiga parcialmente:

- tests en `0444`, para que no los pueda modificar;
- ejecución como uid no privilegiado.

No se elimina sin partir en dos procesos. Para el TP: mitigar lo barato, documentar el residual, y tratar "reporte ausente o mal formado" como estado sospechoso visible al profesor.

#### e) `SecurityManager` no es una opción
...
3. **Acotar el escaneo de classpath de JUnit.** `--scan-class-path` sobre el directorio de clases compiladas, nunca sobre el classpath completo con el jar de JUnit adentro: la diferencia estaba entre 1.2 s y 2.9 s.
4. **Pool caliente de contenedores.** N contenedores ya creados esperando. Llega un job → se arranca uno que ya existe → al terminar **se destruye** y se crea el reemplazo en background. Se ahorra el `create` del camino crítico sin reusar nunca un contenedor entre dos alumnos.
   > **Regla no negociable: un contenedor, una ejecución.**
...
Los relojes, que ahora son dos:

| Reloj | Quién lo fija | Valor propuesto | Qué significa que se venza |
|---|---|---|---|
| `timeoutCompilacionMs` | **Plataforma.** No viaja en el mensaje | 20 000 ms | Toolchain saturado → `ERROR_INTERNO` |
| `timeoutPruebasMs` | **T05 propone**, el sandbox recorta | 10 000 ms (techo) | El código del alumno no termina → `TIMEOUT` |
| Reloj de pared del worker | Plataforma | suma de los dos + margen | Falló el reloj de adentro → `ERROR_INTERNO` |

> **El techo de `timeoutPruebasMs` se fija con el peor caso observado, no con el promedio.** La misma entrega, en corridas seguidas, varió entre 1.8 s y 5.2 s en la fase de tests (§1.4d). Un techo ajustado al promedio produce `TIMEOUT` intermitentes sobre código correcto, que es el modo de falla más caro de diagnosticar y el que peor le queda al alumno.
...
| `TIMEOUT` | El código del alumno no termina | El `timeout` de la fase de tests, adentro del contenedor | **Sí** |
...

{
  ...
  "limites": {                        // propuestos por T05, recortados por el sandbox
    "tiempoPruebasMs": 10000,         // SOLO la fase de tests. Compilar no se
                                      // le cobra al alumno: ese presupuesto es
                                      // de la plataforma y no viaja acá (§1.4a)
    "memoriaMb": 512,
    "cpus":      2.0
  },
  "trazabilidad": {
...
→ 200 OK
{
  ...
  "recursos": {
    "tiempoPruebasMs": 1840,          // lo que tardó el código del alumno
    "tiempoCompilacionMs": 2900,      // costo de plataforma, informativo
    "memoriaPicoKb": 148000
  },
  "limitesAplicados": { "tiempoPruebasMs": 10000, "memoriaMb": 512, "cpus": 2.0 },
  "trazabilidad": { "desafioId": "...", "suiteVersion": 7,
...
`LENGUAJE_NO_SOPORTADO` · `LIMITES_FUERA_DE_RANGO` · `BUNDLE_SIN_TESTS` · `RUTA_INVALIDA` · `SUITE_VERSION_DESCONOCIDA` · `IDEMPOTENCY_KEY_AUSENTE` · `IDEMPOTENCY_CONFLICTO` · `COLA_LLENA` · `TOPE_ALUMNO_EXCEDIDO` · `EJECUCION_INEXISTENTE` · `EJECUCION_NO_CANCELABLE`

`RUTA_INVALIDA` y `SUITE_VERSION_DESCONOCIDA` son los dos códigos nuevos que salieron de probar el prototipo (§1.4). El primero cierra una validación que hoy se apoyaba, sin saberlo, en que el desempaquetador de tar rechazara lo que la API dejó pasar.
