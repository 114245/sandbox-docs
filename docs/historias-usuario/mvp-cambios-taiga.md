# Cambios a cargar en Taiga — segunda pasada del MVP de `ms-sandbox`

## Cómo usar este documento

Este documento lista, ítem por ítem, qué hay que cambiar en las 3 épicas y las 9 historias que ya existen en Taiga. **No se crea ni se borra ningún ítem.**

Cada ítem trae:

1. **Qué cambia**: la lista concreta de cambios, para revisarlos y verificar la carga.
2. **Texto a cargar**: el texto final completo de cada sección del ítem, listo para copiar y pegar en Taiga reemplazando el anterior. Como los cambios tocan casi todas las secciones (incluida la ruta del endpoint), conviene reemplazar la descripción entera en vez de editar frases sueltas.

**Orden de carga:**

1. Actualizar las tres épicas.
2. Actualizar las nueve historias (título, descripción y puntos).
3. Recién al final, cargar las 103 tareas de `mvp-tareas.md`.

**Resumen de puntos:** Épica 1 = 16, Épica 2 = 13, Épica 3 = 34. Total: **63 puntos**.

## Cambios transversales

Estos cambios aparecen en varios ítems a la vez:

- **Ruta del endpoint:** `POST /api/sandbox/executions` y `GET /api/sandbox/executions/{id}`.
- **Salida cruda:** el sandbox no clasifica el resultado ni emite un veredicto de negocio. Devuelve la salida cruda con tope de tamaño y T05 la interpreta.
- **Estados:** la máquina de estados queda con seis estados técnicos: `QUEUED`, `RUNNING`, `COMPLETED`, `TIMEOUT`, `MEMORY_LIMIT` e `INTERNAL_ERROR`.
- **Perfiles:** catálogo estático de tres perfiles sin CRUD: `java21-junit` (con ArchUnit), `java21-pmd` y `java21-checkstyle`.
- **Request:** trae archivos con rol (solución, test o configuración), el identificador de perfil y los identificadores de alumno y de curso. No trae límites.
- **Los tres módulos se construyen desde cero** (`api`, `worker`, `ejecutor`): por eso suben los puntos de HU-01, HU-05 y HU-06.
...
**Estimación agregada:** 16 puntos (sin cambios)

#### Qué cambia

- Suposiciones: el request trae los archivos del bundle con un rol por archivo (solución, test o configuración), el identificador del perfil y los identificadores de alumno y de curso como correlación.
- Suposiciones: el request ya no trae límites de recursos; los define el perfil.
- Restricciones: el alcance usa un catálogo estático de perfiles, sin CRUD.
- Estimación agregada: 16 puntos (HU-01: 8, HU-02: 3, HU-03: 3, HU-04: 2).
...
**Estimación agregada:** 13 puntos

#### Qué cambia

- Objetivo y CA épicos: el alcance incluye construir el worker como componente, con su empaquetado y su imagen.
- CA épico nuevo: el worker registra `RUNNING` y el estado final con la salida cruda directamente en la tabla de ejecuciones, antes de confirmar el mensaje.
- CA épico nuevo: con la cola llena, la API rechaza nuevas entregas con `429` y un tiempo de espera sugerido, sin que el servicio se declare caído.
- Dependencias: se agrega la base de datos del sandbox (el worker escribe estado y resultado).
- Impacto en datos: el worker actualiza las columnas de estado y resultado de la tabla de ejecuciones, con un usuario de base limitado a esas columnas.
...
### Épica 3: Ejecución aislada

**Estimación agregada:** 34 puntos (31 Must + 3 Could)

#### Qué cambia

- Objetivo: el alcance incluye construir el ejecutor, las imágenes de ejecución y el catálogo de perfiles.
- Objetivo y CA épicos: el sandbox devuelve la salida cruda del programa y de la herramienta del perfil; nunca emite un veredicto de negocio (lo interpreta T05).
- Restricciones: catálogo estático de tres perfiles fijos (`java21-junit`, `java21-pmd`, `java21-checkstyle`), sin CRUD, con límites definidos por perfil.
- Estimación agregada: 34 puntos (HU-06: 13, HU-07: 13, HU-08: 5, HU-09: 3).
...
- Descripción (Quiero): la entrega son los archivos que exige el perfil elegido, cada uno con su rol, más el identificador del perfil.
- Reglas de negocio: el request no trae límites de recursos; los identificadores de alumno y de curso se guardan solo como correlación (el de alumno, además, para el tope en simultáneo). Se quita la mención a cancelar en bloque.
- Validaciones: en orden, perfil del catálogo, contenido mínimo según el perfil (`java21-junit`: solución y test; `java21-pmd` y `java21-checkstyle`: solución y configuración), rutas relativas con la extensión de su rol (`.java` o `.xml`) y tope por alumno.
- Datos obligatorios: archivos con rol (solución, test o configuración), identificador de perfil, `Idempotency-Key`, identificadores de alumno y de curso.
- CA2: bundle incompleto para su perfil → `400` `INCOMPLETE_BUNDLE` (reemplaza a `BUNDLE_WITHOUT_TESTS`).
- CA3: solo perfil fuera del catálogo → `422` `UNSUPPORTED_LANGUAGE` (se quita `LIMITS_OUT_OF_RANGE`).
- CA5 nuevo: tope de ejecuciones en simultáneo por alumno → `429` `STUDENT_LIMIT_EXCEEDED`.
- BDD: escenarios 1 y 2 reescritos según el perfil.
- Estimación: el puntaje incluye construir el módulo `api`.
- Todas las rutas del endpoint de ejecuciones pasan a `/api/sandbox/executions` (y `/api/sandbox/executions/{id}`).
...
### HU-02 — Reintentar un envío sin que se ejecute dos veces

**Puntos:** 3 (sin cambios)

#### Qué cambia

- Seguridad: requisito de diseño del ejecutor, sin comparación con un comportamiento actual.
- Todas las rutas del endpoint de ejecuciones pasan a `/api/sandbox/executions` (y `/api/sandbox/executions/{id}`).
...
### HU-03 — Recibir errores claros y estables ante una entrega inválida

**Puntos:** 3 (sin cambios)

#### Qué cambia

- Validaciones: la lista de códigos queda `UNSUPPORTED_LANGUAGE`, `INCOMPLETE_BUNDLE`, `INVALID_PATH`, `RESERVED_PACKAGE`, `MISSING_IDEMPOTENCY_KEY`, `IDEMPOTENCY_CONFLICT`, `QUEUE_FULL`, `STUDENT_LIMIT_EXCEEDED`, `EXECUTION_NOT_FOUND`. Se quitan `LIMITS_OUT_OF_RANGE`, `BUNDLE_WITHOUT_TESTS`, `UNKNOWN_SUITE_VERSION` y `EXECUTION_NOT_CANCELABLE`.
- Notas y riesgos: `INVALID_PATH` es validación explícita de la API, sin referencia a estado de implementación.
- Reglas de negocio: el formato de error se cita como RFC 9457 (reemplaza a la RFC 7807) y se detallan los campos de la respuesta: los estándar (`type`, `title`, `status`, `detail`, `instance`) más el campo `code` con el código estable.
- CA2: también se rechaza una ruta cuya extensión no corresponde a su rol.
- CA3: el paquete reservado aplica solo al perfil `java21-junit`.
- Se elimina el CA de versión de suite (`UNKNOWN_SUITE_VERSION`); el antiguo CA5 (`EXECUTION_NOT_FOUND`) pasa a ser CA4.
- BDD: se elimina el escenario 3 (versión de suite desconocida).
- Todas las rutas del endpoint de ejecuciones pasan a `/api/sandbox/executions` (y `/api/sandbox/executions/{id}`).
...
**Puntos:** 2 (sin cambios)

#### Qué cambia

- Reglas de negocio: el estado es puramente técnico; el sandbox no interpreta el reporte ni emite un veredicto, lo hace T05 sobre la salida cruda.
- Datos obligatorios: máquina de seis estados técnicos (`QUEUED`, `RUNNING`, `COMPLETED`, `TIMEOUT`, `MEMORY_LIMIT`, `INTERNAL_ERROR`).
- Todas las rutas del endpoint de ejecuciones pasan a `/api/sandbox/executions` (y `/api/sandbox/executions/{id}`).
...
- Notas y estimación: el puntaje incluye construir el módulo `worker`.
- Reglas de negocio: el worker escribe `RUNNING` y el estado final con la salida cruda directamente en la base antes de confirmar el mensaje; la API es dueña del esquema y el worker solo actualiza las columnas de estado y resultado.
- CA6 nuevo: cola llena → `429` `QUEUE_FULL` con header `Retry-After`, sin persistir ni encolar la entrega.
- BDD: escenario 4 nuevo (cola llena).
- Todas las rutas del endpoint de ejecuciones pasan a `/api/sandbox/executions` (y `/api/sandbox/executions/{id}`).
...
### HU-06 — Que el código del alumno no pueda llegar a la red, al host ni a otros servicios

**Puntos:** 8 → 13

#### Qué cambia

- Sin cambios: Descripción, Reglas de negocio, Datos obligatorios, Seguridad, Otros, los cinco CA y los tres escenarios del BDD.
- Estimación: 8 → 13, y se agrega el párrafo que explica que el puntaje incluye construir el módulo `ejecutor`, porque se construye desde cero.
- Validaciones: deja de decir que hay que "dejar de parsear" el listado de tar, que describía el código viejo. Ahora dice que la validación es programática y nunca un parseo de texto, y agrega que las rutas deben ser relativas y sin `..`.
- Performance: la referencia pasa de HU-10 a HU-07, por la renumeración.
- Dependencias (Módulos afectados): dice que se crea el módulo `ejecutor`, y la referencia pasa de HU-10 a HU-07.
- Riesgos y mitigación: "integridad del veredicto (ver HU-11)" pasa a "autenticidad de la salida cruda (ver HU-08)", porque el sandbox ya no emite veredicto. Deja de decir que la validación actual del tar es frágil y agrega que, mientras no se decida en qué componente vive esa validación, la historia no puede darse por terminada.
- Prototipo (Mock API): la ruta pasa de `POST /executions` a `POST /api/sandbox/executions`.
...
### HU-07 — Que cada ejecución respete los límites de CPU, memoria y tiempo del perfil

**Puntos:** 8 → 13

#### Qué cambia

- Reglas de negocio: catálogo estático de tres perfiles (`java21-junit` con ArchUnit como librería, `java21-pmd`, `java21-checkstyle`), versionado con las imágenes y cargado al arranque, sin CRUD. Reemplaza al único perfil hardcodeado.
- Reglas de negocio: los límites viven en el perfil; T05 no propone límites.
- Datos obligatorios: límites por perfil; los de `java21-pmd` y `java21-checkstyle` quedan a definir por medición.
- CA6 y CA7 nuevos: perfil desconocido → `UNSUPPORTED_LANGUAGE`; `GET /languages` lista los tres perfiles.
- Estimación: sube por las dos imágenes nuevas y la extensión del catálogo.
- Todas las rutas del endpoint de ejecuciones pasan a `/api/sandbox/executions` (y `/api/sandbox/executions/{id}`).
...
- Título nuevo: "Capturar y devolver la salida cruda del programa y el reporte de la herramienta".
- Toda la historia cambia: el sandbox devuelve stdout, stderr y el/los reporte(s) de la herramienta del perfil, con tope de tamaño, sin interpretarlos. Se quitan el verificador de reporte, el clasificador de veredictos y `EARLY_EXIT`.
- Reglas de negocio: T05 interpreta la salida y es responsable de no mostrarle al alumno la salida de tests ocultos.
- Seguridad: la salida cruda puede ser falsificada por el código del alumno.
- Todas las rutas del endpoint de ejecuciones pasan a `/api/sandbox/executions` (y `/api/sandbox/executions/{id}`).
...
### HU-09 — Un chequeo de salud que distinga "saturado" de "roto"

**Puntos:** 3 (sin cambios)

#### Qué cambia

- Seguridad: requisito del chequeo de salud, sin referencia a un chequeo actual.
- CA2: el `429` de cola llena ahora se define en HU-05; esta historia solo verifica que `liveness` y `readiness` sigan en `UP` con la cola llena.
