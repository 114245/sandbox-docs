# Historias de usuario del MVP — `ms-sandbox`

## Cómo usar este documento

Este archivo reúne, en el formato del template de Taiga del equipo, las historias de usuario
correspondientes al alcance MVP acordado para `ms-sandbox` (Tema 06 — Sandbox / Runtime).
`ms-sandbox` no tiene usuarios humanos directos: su único consumidor es **T05 — Desafíos
Prácticos**, que a su vez le entrega feedback al alumno. Por eso el rol de cada historia es T05,
el operador de la plataforma o el/la responsable de la plataforma, según corresponda, pero el
"Para" siempre apunta al valor que recibe el alumno o el profesor en la cadena.

El equipo debe revisar este documento en conjunto antes de cargarlo a Taiga. Cada historia trae
su estimación en puntos y su prioridad MoSCoW como punto de partida para la discusión, no como
definición cerrada.

Todo dato marcado `[A DEFINIR: ...]` es un campo que todavía no tiene una respuesta fijada y que
el equipo debe cerrar antes de dar la historia por completa.

Los identificadores técnicos (rutas, códigos de error, nombres de eventos, campos y estados) están
en inglés; el texto descriptivo está en español.

Los tres módulos del servicio (`api`, `worker` y `ejecutor`) se construyen desde cero dentro del
alcance de este MVP: ninguno de los tres existe todavía como entregable. Por eso las estimaciones
de las historias que dan origen a un módulo incluyen la construcción completa de ese componente
(su empaquetado, su imagen de contenedor y su lógica), no solo su configuración.

---

## Resumen de historias

| Épica | HU | Título | Puntos | Prioridad |
|---|---|---|---|---|
| Épica 1 — Recepción de ejecuciones (API) | HU-01 | Enviar una entrega y recibir confirmación inmediata | 8 | Must |
| Épica 1 — Recepción de ejecuciones (API) | HU-02 | Reintentar un envío sin ejecutarlo dos veces | 3 | Must |
| Épica 1 — Recepción de ejecuciones (API) | HU-03 | Recibir errores claros y estables ante una entrega inválida | 3 | Must |
| Épica 1 — Recepción de ejecuciones (API) | HU-04 | Consultar el estado y el resultado de una ejecución | 2 | Must |
| Épica 2 — Procesamiento asíncrono (cola y worker) | HU-05 | Que la entrega se evalúe aunque haya un pico o se reinicie un servicio | 13 | Must |
| Épica 3 — Ejecución aislada | HU-06 | Que el código del alumno no pueda llegar a la red, al host ni a otros servicios | 13 | Must |
| Épica 3 — Ejecución aislada | HU-07 | Que cada ejecución respete los límites de CPU, memoria y tiempo del perfil | 13 | Must |
| Épica 3 — Ejecución aislada | HU-08 | Capturar y devolver la salida cruda del programa y el reporte de la herramienta | 5 | Must |
| Opcional (si sobra tiempo) | HU-09 | Un chequeo de salud que distinga "saturado" de "roto" | 3 | Could |
| **Total** | | | **63** | |
...
**Épica:** Recepción de ejecuciones (API)
...
**Épica:** Procesamiento asíncrono (cola y worker)
...
- **Validaciones:** el alcance MVP de esta historia incluye construir una **lista blanca de tipo y nombre de entrada al desempaquetar el bundle**: la validación debe usar una lista blanca de tipos de entrada (solo archivos regulares y directorios), rechazando FIFOs, dispositivos, sockets, symlinks y hard links, y debe ser una validación programática, nunca un parseo de la salida de texto de un listado de tar. Todavía está pendiente decidir en qué componente vive esa validación.
...
**Épica:** Ejecución aislada / Operación
...
## Fuera de alcance del MVP y deuda conocida
